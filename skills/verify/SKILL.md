...
report it as a regression. Check runner contract: per CORE.md — MATCH on
...
**Evidence index.** After filling VERIFICATION.md, (re)generate
`evidence/INDEX.md` on every verdict — a regenerable cache; hand-editing is a
defect, regenerate instead. POSIX sh from repo root, `$d` = the evidence dir:
...
Producer = filename prefix before the first `-`; Truths = numbers of the
VERIFICATION.md truth rows citing the file (`-` = uncited — flag uncited
artifacts); one `DUPLICATE: a == b` line per hash-collision pair.
